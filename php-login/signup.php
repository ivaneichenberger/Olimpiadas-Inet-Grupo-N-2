...
if (!empty($_POST['nombre']) && !empty($_POST['apellido']) && !empty($_POST['fecha_nacimiento']) && !empty($_POST['genero']) && !empty($_POST['direccion']) && !empty($_POST['telefono']) && !empty($_POST['info_medica']) && !empty($_POST['ID_Doctor']) && !empty($_POST['DNI'])) {
    $sql = "INSERT INTO pacientes (Nombre, Apellido, Fecha_Nacimiento, Genero, Direccion, Telefono, Info_Medica, ID_Doctor, DNI) VALUES (:nombre, :apellido, :fecha_nacimiento, :genero, :direccion, :telefono, :info_medica, :ID_Doctor, :DNI)";
